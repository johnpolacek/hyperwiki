...
assert.match(app, /reason: "initial" \| "answer" \| "retry" \| "repair" \| "plan" \| "plan_repair"/);
assert.match(app, /without a parseable question or generated plan/);
assert.match(app, /importTurnSnapshotLineKeys/);
assert.match(app, /hyperwiki-ready-to-plan/);
assert.match(app, /extractLatestReadyToPlanSignal/);
assert.match(app, /importedProjectPlanGenerationPrompt/);
assert.match(app, /importedProjectPlanRepairPrompt/);
assert.doesNotMatch(app, /hyperwiki-local-answer-fallback/);
assert.match(codex, /EXEC_JSON_FIRST_ASSISTANT_TIMEOUT/);

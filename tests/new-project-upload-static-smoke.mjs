...
const source = [await readFile("src/App.tsx", "utf8"), await readFile("src/lib/agent.ts", "utf8"), await readFile("src/components/views/NewProjectView.tsx", "utf8"), await readFile("src/components/terminal/TerminalPane.tsx", "utf8"), await readFile("src/components/terminal/XtermSession.tsx", "utf8"), await readFile("src/lib/terminal.ts", "utf8")].join("\n");
...
assert.ok(source.includes("font-mono") && source.includes("lowercase") && source.includes("hover:bg-transparent hover:text-terminal-muted"), "Terminal pane labels should be lowercase mono and close buttons should have no rollover color shift");
assert.ok(source.includes("border-b border-terminal-border px-3 text-xs") && source.includes('className="terminal-scrollbar-thin h-full min-h-0 p-1"'), "Terminal pane headers and xterm padding should match the compact chrome");
assert.ok(source.includes("border-terminal-border bg-terminal-bg first:border-t-0 not-first:border-t"), "Agent and CLI terminal split borders should be more opaque than the dev chrome.");

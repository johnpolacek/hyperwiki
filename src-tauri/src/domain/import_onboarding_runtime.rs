...
fn plan_repair_prompt(
    ...
    [
        ...
        "Prefer PlanHero for the title/intent, PlanSummary for status/current unit/next action/blockers/validation, PlanUnit for executable unit sections, Decision for accepted choices, Evidence for source-grounded facts, Verification for checks, and Steps/Step for stage or unit sequences.",
...
fn generic_unit_artifact(
    ...
    GeneratedPlanArtifact {
        path: path.to_string(),
        content: format!(
            "---\ntitle: \"{title}\"\ndescription: \"Source-grounded MVP implementation unit.\"\nwikiKind: \"plan\"\n---\n\n<PlanHero><h1>{title}</h1><p>Intent: {intent}</p></PlanHero><PlanUnit title=\"Scope\"><p>Stay inside the accepted imported-project MVP surface and avoid unrequested product, service, framework, deployment, account, or integration scope.</p></PlanUnit><PlanUnit title=\"Implementation Notes\"><p>Use the accepted source decisions as authority, but keep visible instructions concise.</p></PlanUnit><Evidence title=\"Accepted source decisions\"><p>Full context is preserved for agents.</p><Visibility for=\"agents\">{decision_summary}</Visibility></Evidence><Decision title=\"Dependencies\"><p>Depends on prior units in Stage 01 and the accepted import Q&amp;A decisions. Blockers: none unless implementation discovers a contradiction with source evidence.</p></Decision><Verification><p>Run applicable repository checks and manually exercise the behavior named by the imported source.</p></Verification><PlanUnit title=\"Completion Gate\"><p>Complete when the unit behavior is implemented, verified, and no source decision has been contradicted.</p></PlanUnit>"
        ),
    }
}
...
"#;
    ...
    fn deterministic_compiler_generates_valid_static_local_mvp_plan() {
        ...
        assert!(joined.contains("<PlanUnit title=\"Implementation Notes\">"));

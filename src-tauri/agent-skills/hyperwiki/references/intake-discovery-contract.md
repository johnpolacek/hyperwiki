...
Use `intake_discovery` when the user wants to start a software project but has no repo, no PRD, no notes, no project idea, or only a vague prompt such as "I want to build something." This mode gathers enough user-confirmed direction before the Project MDX Wiki Skill creates any wiki artifacts.
...
- Explicit unknowns to preserve in `wiki/sources.mdx`

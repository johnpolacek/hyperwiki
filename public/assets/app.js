...
function renderProjectSwitcher(groups) {
  projectList.replaceChildren(...groups.map((group) => {
    ...
    button.addEventListener("pointerdown", () => {
      ...
      setTimeout(closeTopbarPanels, 150);
      setTimeout(() => void switchProject(project), 160);
    });
...
async function createProjectFromMarkdown() {
  ...
  try {
    ...
    const prompt = [
      ...
      "- Use the imported source first, then ask a detailed Q&A session in this terminal for maximum clarity before writing implementation stages or units.",
      "- Do not accept generic placeholder stages or units. Create stages only around real phase gates and create units only when each unit has concrete scope, acceptance criteria, verification, and completion gates.",
      "- Create or revise source briefs, roadmap, and planning pages only where the document plus Q&A answers support durable project context.",
      "- Ask about product scope, first user workflow, non-goals, architecture, data, provider/API dependencies, safety/privacy, validation, and rollout whenever those decisions are not already settled.",
